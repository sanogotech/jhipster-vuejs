...
export default class Consultant extends mixins(JhiDataUtils, AlertMixin) {
  ...
  public retrieveAllConsultants(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.consultants.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IConsultant): void {
  ...
  public removeConsultant(): void {
    this.consultantService()
      .delete(this.removeId)
      .then(() => {
        const message = this.$t('jhipsterApp.consultant.deleted', { param: this.removeId });
